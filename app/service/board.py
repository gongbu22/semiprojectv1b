...
class BoardService:
    ...
    def find_select_board(db, ftype, fkey, cpg):
        try:
            ...
            elif ftype == 'titcont': myfilter = or_(Board.contents.like(fkey), Board.contents.like(fkey))
    ...
    def selectone_board(bno, db):
        try:
            ...
            print(f'▶▶▶ selectone_board 오류 발생 : {str(ex)}')
            db.rollback()
    ...
    def insert_reply(db, rp):
        try:
            ...
            next_rno = db.execute(stmt).scalar_one()  # 단일값
