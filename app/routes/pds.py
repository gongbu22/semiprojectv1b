...
async def pdsdown(db: Session = Depends(get_db)):
    ...
    return StreamingResponse(iterfile(), media_type='application/octet-stream',
                             headers={"Content-Disposition": f"attachment; filename={down_fname}"})  # 다운로드시 파일 이름 지정
...
async def mp3play(req: Request):
    ...
    return StreamingResponse(iterfile(), media_type='audio/mp3')  # 오디오 파일만 보내는거라서 fname 설정X